...
static int match(u32 tid, struct sockaddr *sa, char *initiator, char *filename)
{
	...
	if (!(fp = fopen(fname, "r"))) {
		...
		snprintf(fname, sizeof(fname), "%s%s", ETC_DIR, filename);
		fp = fopen(fname, "r");
		if (!fp)
			return -errno;
	}
...
static void plain_init(char *params, int *timeout)
{
	...
	snprintf(file2, sizeof(file2), ETC_DIR "%s", INI_DENY_FILE);
	...
	snprintf(file2, sizeof(file2), ETC_DIR "%s", CONFIG_FILE);
