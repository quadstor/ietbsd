...
#if 0
	if (!(conn->sock->so_rcv.sb_state & SBS_CANTRCVMORE))
		return 0;
#endif
...
#else
static inline int
sk_wspace_available(struct iscsi_conn *conn)
{
	struct file filetmp;
	int error, avail = 0;
...
static void conn_reset_nop_timer(struct iscsi_conn *conn)
{
	...
	if (target->trgt_param.nop_interval)
		callout_reset(&conn->nop_timer,
			      HZ * target->trgt_param.nop_interval,
...
static void istd(void *arg)
{
	...
	for (;;) {
		...
		wait_event_interruptible(info->nthread_wait, test_and_clear_bit(D_DATA_READY, &info->flags) || test_bit(D_THR_EXIT, &info->flags));

		if(test_bit(D_THR_EXIT, &info->flags))
			break;
...
int nthread_stop(struct iscsi_target *target)
{
	...
	wakeup(&info->nthread_wait);
	msleep(info->task, &info->nthread_wait.chan_lock, 0, "wait for istd exit", 0);
