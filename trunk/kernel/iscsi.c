	...
	case WRITE_SAME_16:
	{
		...
		if (req_hdr->scb[0] == WRITE_SAME_16) {
			dprintk(D_VAAI, "WRITE_SAME_16: D: %u L:%u O:%llu\n",
				cmnd_write_size(req), length, (unsigned long long)offset);
...
out:
	iprintk("%s (%02x) issued on tid:%d lun:%d by sid:%llu (%s)\n",
				...
				translate_lun(req_hdr->lun), (unsigned long long)session->sid,
...
#else
	callout_reset(&cmnd->req->timer, HZ * t, nop_in_timeout, cmnd->req);
#endif
}
...
#else
	iscsi_cmnd_cache = uma_zcreate("ietcmndcache", sizeof(struct iscsi_cmnd), NULL, NULL, NULL, NULL, 0, 0);
#endif
	if (!iscsi_cmnd_cache)
		goto err;
...
MALLOC_DEFINE(M_IET, "iet", "IET allocations");
