...
static void gen_scsiid(struct iet_volume *volume)
{
	...
	MD5Init(&ctx);
	MD5Update(&ctx, volume->target->name, strlen(volume->target->name));
	MD5Update(&ctx, &volume->lun, sizeof(volume->lun));
	MD5Final(volume->scsi_id, &ctx);
}
#endif
...
int volume_add(struct iscsi_target *target, struct volume_info *info)
{
	...
	ret = copy_from_user(args, (void *)(unsigned long)info->args_ptr,
			     info->args_len);
#else
	ret = copyin((void *)(unsigned long)info->args_ptr, args,
			     info->args_len);
#endif
	if (ret) {
		ret = -EFAULT;
...
int is_volume_reserved(struct iet_volume *volume, u64 sid, u8 *scb)
{
	...
	bool_t write_excl = false;
	bool_t excl_access = false;
	bool_t write_excl_ro = false;
	bool_t excl_access_ro = false;
